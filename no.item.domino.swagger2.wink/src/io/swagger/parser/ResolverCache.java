...
// For java 1.6 (utvidet regex bibliotek)
...
public class ResolverCache {
   ...
   // private static final Pattern RESPONSE_PATTERN = Pattern.compile("^" + RefType.RESPONSE.getInternalPrefix() + "(?<name>.+)");
	...
	//Kode for Java 1.6
	private static final NamedPattern PARAMETER_PATTERN = NamedPattern.compile("^" + RefType.PARAMETER.getInternalPrefix() + "(?<name>.+)");	
	private static final NamedPattern DEFINITION_PATTERN = NamedPattern.compile("^" + RefType.DEFINITION.getInternalPrefix() + "(?<name>.+)");
	private static final NamedPattern RESPONSE_PATTERN = NamedPattern.compile("^" + RefType.RESPONSE.getInternalPrefix() + "(?<name>.+)");
    ...
    /**
     * Metode for å kunne benytte lik logikk på java versjon 1.6
     * Har derfor benyttet meg av biblioteket com.google.code.regexp
     * 
     * @param ref
     * @param map
     * @param pattern
     * @return
     */
    private Object getFromMap(String ref, Map<?, ?> map, NamedPattern pattern) {
        final NamedMatcher parameterMatcher = pattern.matcher(ref);
...
/*  Virker i Java 1.7, virker ikke i Java 1.6 pga. parameterMatcher.group("name");
 *   
 * 
